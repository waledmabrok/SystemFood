...
class ProductRepository {
  ...
  Future<List<Product>> getAll({bool activeOnly = false, String? categoryId}) async {
    String? where;
    ...
    final results = await _db.query('products', where: where, whereArgs: whereArgs, orderBy: 'name ASC');
  ...
  Future<List<Product>> getLowStock() async {
    final results = await _db.rawQuery(
      'SELECT * FROM products WHERE is_active = 1 AND min_stock > 0 AND stock <= min_stock ORDER BY stock ASC',
    );
    return results.map(Product.fromMap).toList();
  }
