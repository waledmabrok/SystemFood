          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const Spacer(),
                Text('${_orders.length} طلب', style: AppTypography.bodySmall.copyWith(color: AppColors.textSecondary)),
              ],
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            Expanded(flex: 2, child: Text('#${o.orderNumber}', style: AppTypography.bodySmall.copyWith(fontWeight: FontWeight.w700, color: AppColors.primary))),
            Expanded(flex: 2, child: Text(_formatDateLocal(o.createdAt), style: AppTypography.bodySmall.copyWith(color: AppColors.textSecondary))),
            Expanded(flex: 1, child: Text(_formatTimeLocal(o.createdAt), style: AppTypography.bodySmall.copyWith(color: AppColors.textSecondary))),
            Expanded(flex: 2, child: Text(o.userName ?? '—', style: AppTypography.bodySmall)),
            Expanded(flex: 2, child: Text('${o.finalAmount.toStringAsFixed(2)} ${AppStrings.currency}', style: AppTypography.bodySmall.copyWith(fontWeight: FontWeight.w600))),
            Expanded(flex: 2, child: Text(o.paymentMethod.label, style: AppTypography.bodySmall)),
            Expanded(flex: 2, child: _OrderStatusBadge(o.status)),
            SizedBox(
              width: 80,
              child: TextButton(onPressed: widget.onView, child: const Text('عرض', style: TextStyle(fontSize: 13))),
            ),
          ],
        ),
      ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text('تفاصيل الطلب #${order.orderNumber}', style: AppTypography.headlineSmall),
                  const Spacer(),
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (order.paymentRef != null) _InfoRow('الرقم المرجعي', order.paymentRef!),
                      ...
                      ...order.items.map((item) => Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Text('× ${item.quantity.toStringAsFixed(0)}', style: AppTypography.bodyMedium.copyWith(color: AppColors.textSecondary)),
                            const SizedBox(width: 16),
                            Text('${item.totalPrice.toStringAsFixed(2)} ${AppStrings.currency}', style: AppTypography.bodyMedium.copyWith(fontWeight: FontWeight.w600)),
                          ],
                        ),
                      )),
                      ...
                      if (order.discountAmount > 0)
                        _InfoRow(AppStrings.posDiscount, '-${order.discountAmount.toStringAsFixed(2)} ${AppStrings.currency}'),
                      _InfoRow(AppStrings.posTotal, '${order.finalAmount.toStringAsFixed(2)} ${AppStrings.currency}', bold: true),
                      if (order.changeAmount > 0)
                        _InfoRow(AppStrings.paymentChange, '${order.changeAmount.toStringAsFixed(2)} ${AppStrings.currency}', color: AppColors.success),
                    ],
                  ),
                ),
              ),
